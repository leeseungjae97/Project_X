...
UTXLockOnComponent::UTXLockOnComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	LockOnWidgetComponent = CreateDefaultSubobject<UWidgetComponent>(TEXT("Lock On Widget Component"));
}

void UTXLockOnComponent::BeginPlay()
{
	Super::BeginPlay();
}

void UTXLockOnComponent::DestroyComponent(bool bPromoteChildren)
{
	Super::DestroyComponent(bPromoteChildren);
}

void UTXLockOnComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
 ...
 *	3. 없으면 LockOn 해제
 */
void UTXLockOnComponent::TraceEnemy(float DeltaTime)
{
	if (!OwnerCharacter)
		return;
...
void UTXLockOnComponent::HandleLockOnTarget(float DeltaTime)
{
	if (!bBeforeLockOn)
	{
		bBeforeLockOn = true;
	...
	if (ITXLockOnTargetInterface* LockOnTarget = Cast<ITXLockOnTargetInterface>(TargetActor))
	{
 ...
 */
void UTXLockOnComponent::SetVisibilityLockOn(bool InbVisibility)
{
...
void UTXLockOnComponent::TraceLockOnTarget()
{
	...
	if (LineTrace(EyeLoc, EyeRot, LockOnTraceDistance, HitResult, {}, { ECC_Pawn, ECC_WorldDynamic, ECC_WorldStatic, ECC_Visibility })
		&& HitResult.GetActor())
	{
		if (HitResult.GetActor()->Implements<UTXLockOnTargetInterface>())
		{
 ...
 */
bool UTXLockOnComponent::IntervalTargetTrace(FHitResult& HitResult, float DeltaTime)
{
	if (!TargetActor)
	{
		ResetTarget();
	...
	if (TraceIntervalTime <= TraceIntervalElpsedTime)
	{
		...
		if (LineTrace(EyeLoc, EyeRot, LockOnTraceDistance, HitResult, {}, { ECC_Pawn, ECC_WorldDynamic, ECC_WorldStatic, ECC_Visibility }))
		{
			if (HitResult.GetActor() && HitResult.GetActor()->Implements<UTXLockOnTargetInterface>())
			{
...
void UTXLockOnComponent::ResetTarget()
{
	if (ITXLockOnTargetInterface* LockOnTarget = Cast<ITXLockOnTargetInterface>(TargetActor))
	{
...
void UTXLockOnComponent::ChangeLockOnTarget(FHitResult& HitResult)
{
...
void UTXLockOnComponent::ChangeLockOnTarget(AActor* Actor)
{
	if (!Actor)
		return;
...
void UTXLockOnComponent::UpdateDetectedActorsInRange()
{
	...
	if (World->OverlapMultiByObjectType(OverlapResults, OwnerCharacter->GetActorLocation(), FQuat::Identity, ObjectParams, FCollisionShape::MakeSphere(UpdateLockOnTraceDistance), Params))
	{
		...
		{
			if (Overlap.GetActor()->Implements<UTXLockOnTargetInterface>())
			{
...
/**@deprecated*/
void UTXLockOnComponent::MakeIndicatorArray()
{
	if (!IndicatorHUDWidget)
		return;
...
void UTXLockOnComponent::PickDetectedRangeActors()
{
...
void UTXLockOnComponent::UpdateDetectDistance(float Distance)
{
	UpdateLockOnTraceDistance = FMath::Clamp(Distance, 0, LockOnTraceDistance);
}

bool UTXLockOnComponent::LineTrace(const FVector Start, const FRotator Direction, float Distance, FHitResult& HitResult, const TArray<AActor*>& ActorsToIgnore, const TArray<TEnumAsByte<ECollisionChannel>>& QueryObjectTypes, FLinearColor TraceColor, FLinearColor TraceHitColor, float DrawTime)
{
	if (!OwnerCharacter)
		return false;
 ...
 */
bool UTXLockOnComponent::ConeTraceMulti(const FVector Start, const FRotator Direction, float ConeHeight, float ConeHalfAngle, const TArray<TEnumAsByte<ECollisionChannel>>& QueryObjectTypes, bool bTraceComplex, const TArray<AActor*>& ActorsToIgnore, bool bIgnoreSelf, float DrawTime)
{
	if (!OwnerCharacter)
		return false;
 ...
 */
void UTXLockOnComponent::LockOn()
{
...
void UTXLockOnComponent::SetOwnerCharacter(ACombatCharacter* InOwnerCharacter)
{
