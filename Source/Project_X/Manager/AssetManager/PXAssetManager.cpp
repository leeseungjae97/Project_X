...
#include "PXAssetManager.h"

#include "PXPrimaryDataAsset.h"
#include "Kismet/GameplayStatics.h"
#include "UI/TXLoadingWidget.h"

FString UPXAssetManager::MakeBlueprintGeneratedClassPath(const FString& InBasePath)
{
...
UPXAssetManager& UPXAssetManager::Get()
{
	check(GEngine);

	UPXAssetManager* Singleton = Cast<UPXAssetManager>(GEngine->AssetManager);
	if (Singleton)
	{
		return *Singleton;
	}
	...
	return *NewObject<UPXAssetManager>();
}

void UPXAssetManager::StartInitialLoading()
{
	Super::StartInitialLoading();
}

void UPXAssetManager::StartAssetPreLoading(UObject* WorldContextObject)
{
	...
	{
		auto* DataAsset = Cast<UPXPrimaryDataAsset>(DataAssetPath.ResolveObject());
...
UClass* UPXAssetManager::LoadBPClass(FString RepPath)
{
	if(RepPath.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("UPXAssetManager::LoadBPClass: Invalid RepPath"));
		return nullptr;
	}
	...
	if(!LoadedClass)
	{
		UE_LOG(LogTemp, Error, TEXT("UPXAssetManager::LoadBPClass: Invalid RepPath. Class Load is failed. Path is %s"), *RepPath);
		return nullptr;
	}
...
void UPXAssetManager::LoadBPClassAsync(FString RepPath, TFunction<void(UClass*)> OnLoaded)
{
	if (RepPath.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("UPXAssetManager::LoadBPClassAsync: Empty RepPath"));
		OnLoaded(nullptr);
		return;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("UPXAssetManager::LoadBPClassAsync: Invalid ClassPath: %s"), *RepPath);
		OnLoaded(nullptr);
		return;
	}
	...
	GetStreamableManager().RequestAsyncLoad(
		...
		[ClassPath, OnLoaded]()
		{
			...
			if (!LoadedClass)
			{
				UE_LOG(LogTemp, Error, TEXT("UPXAssetManager::LoadBPClassAsync: Failed to resolve class: %s"), *ClassPath.ToString());
			}
