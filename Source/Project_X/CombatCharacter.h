...
class UPXCombatComponent;
class UPXPlayerAnimInstance;
...
class PROJECT_X_API ACombatCharacter : public ACharacter, public ICombatAttacker, public IDamageable
{
	...
	UPXCombatComponent* TXCombatComponent;

	UPROPERTY()
	UPXPlayerAnimInstance* TXPlayerAnimInstance;

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = Combat, meta = (AllowPrivateAccess = "true"))
	UPXLockOnComponent* TXLockOnComponent;
...
public:
	...
	TSubclassOf<APXWeapon> MeleeWeaponClass;

	UPROPERTY(EditAnywhere)
	TSubclassOf<APXWeapon> HitscanWeaponClass;
